...
// GET /api/orders  — supports ?customer_id=X filter
...
router.post('/', async (req, res) => {
  try {
    ...
    // Validate customer
    const customer = await User.findById(Number(customer_id)).select('-password').lean();
    if (!customer) return res.status(404).json({ message: 'Customer not found' });

    // Enrich items with product details and deduct stock
...
// PUT /api/orders/:id — update status, notes, delivery_date
