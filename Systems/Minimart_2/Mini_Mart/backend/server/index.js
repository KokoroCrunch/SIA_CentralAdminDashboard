...
// Try Atlas first; if it fails within 5 seconds, fall back to local MongoDB (Compass)
async function connectDB() {
  const ATLAS_URI = process.env.MONGO_URI;
  const LOCAL_URI = process.env.LOCAL_MONGO_URI || 'mongodb://localhost:27017/minimart';

  try {
    await mongoose.connect(ATLAS_URI, { serverSelectionTimeoutMS: 5000 });
    console.log('MongoDB Connected — Atlas');
  } catch (atlasErr) {
    console.warn(`Atlas unavailable (${atlasErr.message})`);
    console.log('Falling back to local MongoDB (Compass)...');
    try {
      await mongoose.connect(LOCAL_URI, { serverSelectionTimeoutMS: 5000 });
      console.log('MongoDB Connected — Local (localhost:27017)');
    } catch (localErr) {
      console.error('Local MongoDB also unavailable:', localErr.message);
      console.error(
        'No database connection. The server will start but all DB operations will fail.',
      );
    }
  }
}
